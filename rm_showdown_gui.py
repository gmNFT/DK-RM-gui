...
fname = f'dk_{gday}_week{week}.csv'
...
# G25 -> skiprows = 24, usecols = 6: 20

data = pd.read_csv(ffolder + fname, skiprows = 24, usecols = range(6, 20), header = 0)
...
df_merge = df_merge[['Name', 'Position_x', 'TeamAbbrev', 'Card Rarity', 'DK', 'Card Set', 'Card Edition', 'Unique ID']]
df_merge.columns = ['Name', 'Pos', 'Team', 'Rarity', 'Proj', 'Set', '#', 'ID']
...
# Create initial data
categories = ['CPT', 'FLEX1', 'FLEX2', 'FLEX3']
Ngroups = 26
groups = []
...
for i, (n, p, r, x, u, m) in enumerate(zip(df['Name'], df['Pos'], df['Rarity'], df['Proj'], df['ID'], df['Team'])):
    ...
    buttons.append((pygame.Rect(x_offset, row * 40, 90, 30), (n, x, u, m), color))
...
for group in groups:
    ...
    for category in categories:
        ...
        if category == 'CPT':
            ...
            ccolor = (100, 100, 100)
        cells[group][category] = {'rect': cell, 'names': [], 'projs': [], 'ids': [], 'teams': [], 'color': ccolor}
        x += cell_width + 20
    y += cell_height + 10
...
if importSpreadsheet:
    f_import = ffolder + f'dk_{gday}_week{week}_lineups.csv'
    df_imp = pd.read_csv(f_import)

    Nrows = len(df_imp['CPT_id'])

    for nr in range(Nrows):

        positions = ['CPT', 'FLEX1', 'FLEX2', 'FLEX3']
        ...
        for pos in positions:
            ...
            for button_data in buttons:
                button, (n, x, u, m), color = button_data
                if u == posID:
                    # print(n)
...
while True:
    for event in pygame.event.get():
        ...
        # MOUSEBUTTONDOWN
            ...
            # Drag
                ...
                for button, (name, proj, uid, team), color in buttons:
                    ...
                    if scrolled_button.collidepoint(pos):
                        ...
                        dragging = (pygame.Rect(scrolled_button), name, proj, uid, team, color)
                ...
                if not dragging_from_column:
                    for group, categories in cells.items():
                        for category, cell_data in categories.items():
                            cell, names, projs, ids, teams, color = cell_data['rect'], cell_data['names'], cell_data['projs'], cell_data['ids'], cell_data['teams'], cell_data['color']
                            if cell.collidepoint(pos) and names:  # If the cell contains names
                                # original_cell_position = cell.topleft # original cell position
                                dragging = (pygame.Rect(cell), names[-1], projs[-1], ids[-1], teams[-1], color)  # Pick up the last values from this cell
        ...
        # MOUSEBUTTONUP
                ...
                if dragging_from_column:
                    for group, categories in cells.items():
                        for category, cell_data in categories.items():
                            ...
                            if cell.collidepoint(pos):
                                ...
                                teams.append(dragging[4])
                                cell_data['color'] = dragging[5]  # Set the color to match the button
                    ...
                    for button, (name, proj, uid, team), color in buttons:
                        ...
                        if scrolled_button.collidepoint(pos):  # If the name is dropped back on the left column
                            buttons.append((pygame.Rect(button), (dragging[1], dragging[2], dragging[3]), dragging[4]))  # Add it back to buttons
                            dragging_from_cell = False
                            break  # Stop checking further buttons
            ...
            if event.key == pygame.K_s:  # When 's' key is pressed save groups to csv
                ...
                df_data = {'CPT': [], 'FLEX1': [], 'FLEX2': [], 'FLEX3': [], 'CPT_id': [], 'FLEX1_id': [], 'FLEX2_id': [], 'FLEX3_id': []}
                ...
                for group, categories in cells.items():
                    ...
                    for position in ['CPT', 'FLEX1', 'FLEX2', 'FLEX3']:
                ...
                df_export.to_csv(ffolder + f'dk_{gday}_week{week}_lineups.csv', index=False)
    ...
    for button, (name, proj, uid, team), color in buttons:
        ...
        if 0 <= scrolled_button.top <= height:
            ...
            label2 = font.render(team, True, (0, 0, 0))
    ...
    for group, categories in cells.items():
        for category, cell_data in categories.items():
            ...
            if cell_data['names']:
                ...
                screen.blit(label, (cell_data['rect'].x + 10, cell_data['rect'].y + 7))

    # Draw loop for Dragging
    if dragging:
        mouse_x, mouse_y = pygame.mouse.get_pos()
        dragging[0].topleft = (mouse_x - 50, mouse_y - 15)
        pygame.draw.rect(screen, dragging[5], dragging[0])  # Use dragging[5] for the color
        font = pygame.font.SysFont(None, fsize)
